...
DEFAULT_HOVER_MM  = 8.0     # pen–sensor gap
DEFAULT_TILT      = 5.0
...
class SensorGrid:
    ...
    def estimate_pos_gauss_fit(self, local):
        """Log-linear Gaussian peak fit (advanced method)."""
        pts = [(s.x, s.y, s.value) for s in local if s.value > 0.001]
        if len(pts) < 4:
            return None
        X, y = [], []
        for sx, sy, v in pts:
            X.append([1, sx, sy, sx * sx + sy * sy])
            y.append(math.log(v))
        try:
            p = np.linalg.lstsq(np.array(X), np.array(y), rcond=None)[0]
            if p[3] >= 0:
                return None
            ex = -p[1] / (2 * p[3])
            ey = -p[2] / (2 * p[3])
            # Reject estimates outside grid bounds + 1 pitch margin
            b = self.bounds
    ...
    def estimate_pos_centroid(self):
        """Firmware-exact weighted centroid (sensor.rs find_center).
        Square: grid indices as firmware, then scaled to mm.
        Hex: physical coords (no firmware analogue).
        """
        total = 0
        sum_x = 0.0
        sum_y = 0.0
        for i, s in enumerate(self.sensors):
            ...
            if self.grid_type == "hex":
                ...
                # Exact firmware: sum_x += val * (i % COL_LEN)
                sum_x += v * (i % self.cols)
                sum_y += v * (i // self.cols)
        if total == 0:
            return None
        ...
        # Map grid indices → mm (firmware maps to 0..MAX for USB HID)
        return ((sum_x / total) * self.pitch,
                (sum_y / total) * self.pitch)

    def apply_filter(self, new, mouse_mm=None):
        if new is None:
            # Clear stale cursor when mouse has moved far from last estimate
            if self.est_pos is not None and mouse_mm is not None:
                dist = math.hypot(mouse_mm[0] - self.est_pos[0],
                                  mouse_mm[1] - self.est_pos[1])
                if dist > self.pitch * 2:
                    self.est_pos = None
            return self.est_pos
        # Reject wild jumps UNLESS the new estimate is near the mouse
        if self.est_pos is not None:
            jump = math.hypot(new[0] - self.est_pos[0],
                              new[1] - self.est_pos[1])
            if jump > self.pitch * 3:
                if mouse_mm is not None:
                    near = math.hypot(new[0] - mouse_mm[0],
                                      new[1] - mouse_mm[1])
                    if near < self.pitch * 2:
                        self.est_pos = new   # validated fast-move snap
                        return self.est_pos
                return self.est_pos   # genuine outlier
        if self.est_pos is None:
            self.est_pos = new
        else:
            a = self.alpha
            self.est_pos = (self.est_pos[0] + a * (new[0] - self.est_pos[0]),
                            self.est_pos[1] + a * (new[1] - self.est_pos[1]))
        return self.est_pos
...
class App:
    def __init__(self):
        ...
        self.rand_speed  = 1.0          # random tilt speed multiplier
        ...
        self.show_centroid = True      # show centroid marker
        self.target_w  = 180.0
        self.target_h  = 100.0
    ...
    def update(self) -> List[Sensor]:
        ...
        if self.mouse_in_grid:
            ...
            # Mouse left the grid → clear estimates to avoid stale markers
            self.grid.est_pos = None
            self.grid.est_pos_centroid = None
            self.grid.clear_field()
        local = self.grid.find_local(12)
        # Gaussian fit filtered (pass mouse pos so filter can validate jumps)
        mouse_hint = self.cursor_mm if self.mouse_in_grid else None
        self.grid.apply_filter(self.grid.estimate_pos_gauss_fit(local), mouse_hint)
        # Firmware centroid (unfiltered)
        self.grid.est_pos_centroid = self.grid.estimate_pos_centroid()
    ...
    def _draw_grid_view(self):
        ...
        if self.tilt > 0.5 and self.mouse_in_grid:
            cx2, cy2 = self.mm2scr(*self.cursor_mm)
            arr_len = 30
            ax = cx2 + arr_len * math.cos(math.radians(self.tilt_dir))
            ay = cy2 + arr_len * math.sin(math.radians(self.tilt_dir))
